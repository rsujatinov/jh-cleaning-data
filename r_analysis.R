...
# Data for this project if broken down into two different groups: training data and test data.
# Furthermore, withing each group, data is stored in three different files:
#    - X_test.txt (X_train.txt)
#    - y_test.txt (y_train.txt)3
...
x.test = read.table("./UCI HAR Dataset/test/X_test.txt", col.names = features$feature)
x.train = read.table("./UCI HAR Dataset/train/X_train.txt", col.names = features$feature)
...
#     2. Across test and training groups: merged datasets from step 1 have the same columns, and they should be
#        combined in a single dataset by combining rows
...
# Also, we see that many column names are not well formatted, let's make them more user friendly,
# by specifying "time" instead of "t" prefix, "freq" instead of "f" prefix, filtering out "..." and "..", 
# and removing "." in the end of column names:
...
# Finally, we need to create a second independent tidy data set
